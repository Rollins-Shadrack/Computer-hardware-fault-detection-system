...
faults = [row['pest'].lower() for row in dataset]

# Create a list of rules based on the dataset
rules = []
for row in dataset:
    rule = {
        'symptoms': [row['species'].lower(), row['control_method'].lower(), row['danger_level'].lower()],
...
def diagnose():
    ...
    if knows_issue.lower() == 'yes':
        ...
        if not diagnoses:
            ...
            if matches:
                diagnoses = [row['diagnosis'] for row in dataset if row['pest'] in matches]
...
welcome_label = tk.Label(root, text='Welcome to Pest and disease control system')
...
issue_label = tk.Label(root, text='Do you know which pest is affecting your Plants ? (Yes/No):')
...
cpu_ram_label = tk.Label(root, text='Which species is the pest:')
cpu_ram_entry = tk.Entry(root, width=50)

hard_disk_drive_label = tk.Label(root, text='which control method can control it?:')
hard_disk_drive_entry = tk.Entry(root, width=50)

display_label = tk.Label(root, text='What is the danger level [high/low/moderate]:')
